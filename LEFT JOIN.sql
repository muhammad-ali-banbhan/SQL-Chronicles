...
-- LEFT JOIN (ya LEFT OUTER JOIN) INNER JOIN se thoda mukhtalif hai. Yaad hai INNER JOIN sirf woh records deta hai jo dono tables mein match karte hain? LEFT JOIN ka concept isse aage badhta hai.
...
-- Agar left table ke kisi record ke liye right table mein koi matching record nahi milta, toh bhi left table ka record toh return hoga, lekin right table ke columns ke liye NULL values show hongi.
...
-- Un saare customers ka customerName, customerNumber, aur agar unhone koi order kiya hai toh us order ka orderNumber aur orderDate nikalen. Agar kisi customer ne koi order nahi kiya, tab bhi woh list mein shamil hona chahiye.

-- Question 2: All Products and Their Order Details (Even if Not Ordered) (products & orderdetails tables)
-- Saare productName, productLine nikalen, aur agar woh product kisi order mein shamil hai toh us order ki orderNumber aur quantityOrdered bhi show karein. Un products ko bhi dikhayen jo abhi tak kisi order mein nahi hain.

-- Question 3: Employees and Their Assigned Customers (Including Those Without Customers) (employees & customers tables)
-- Har employee ka firstName, lastName, aur agar usne koi customer handle kiya hai toh us customer ka customerName nikalen. Un employees ko bhi dikhayen jinhe abhi tak koi customer assign nahi hua hai.
